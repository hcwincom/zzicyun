...
class StoreInModel extends Model
{
     ...
     */
    public function instore_back($info,$admin,$rdsc='还原已审核入库'){
        //统一文件锁
        $file = "log/store_lock.txt";
        $fp = fopen($file, "r");
        if (flock($fp, LOCK_EX )){
            ...
            if($row===1){
                ...
                if($info['status']==2){
                    ...
                    $row=$m_goods->where('id',$info['goods'])->dec('store_num',$info['num'])->update($update_info);
                    if($row!==1){
                        $row='库存信息更新失败，请刷新重试';
                    }
                }
