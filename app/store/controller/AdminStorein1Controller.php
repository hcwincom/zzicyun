...
class AdminStorein1Controller extends AdminBaseController
{
    protected $m;
    protected $statuss;
    ...
    public function index()
    {
        ...
        $where_num=1;
        $nums=['p.num'=>'入库数量','(p.num*goods.price1)' => '总金额'];
        ...
        $field='p.*,shop.name as sname,goods.name as goods_name,goods.code as goods_code'.
            ',(p.num*goods.price1) as money,a.user_nickname as aname,r.user_nickname as rname';
       ...
       //whereRaw运行原始sql
        ...
        //分类
        $m_cate=Db::name('goods_cate');
        $where_cate=[
            'fid'=>0,
            'status'=>2,
        ];
        $cates0=$m_cate->where($where_cate)->order('sort asc')->column('id,name,code');
        $where_cate=[
            'status'=>['eq',2],
            'fid'=>['gt',0],
        ];
